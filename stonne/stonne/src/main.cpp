...
//    stonne_cfg.m_MSNetworkCfg.ms_size=128;
    ...
    // 动态分配内存给浮点型数组，并将它们的指针赋值给MK_matrix等
    ...
    //Computing the CNN Layer with the simulator
    std::cout<<std::endl;
    std::cout<<"Computing the CNN Layer with the simulator (main.cpp)"<<std::endl;
    std::cout<<std::endl;

    std::cout<<std::endl;
    std::cout<<"1. Instantiate a stonne object ： stonne_instance (main.cpp)"<<std::endl;
    std::cout<<std::endl;
    Stonne* stonne_instance = new Stonne(stonne_cfg); //Creating instance of the simulator
