...
void VNAT_Register::update() {
        ...
        this->addr = this->base_addr + (index_N)*dnn_layer->get_X_()*dnn_layer->get_Y_()*dnn_layer->get_K()*dnn_layer->get_G()*STONNE_WORD_SIZE + index_X*dnn_layer->get_Y_()*dnn_layer->get_K()*dnn_layer->get_G()*STONNE_WORD_SIZE + index_Y*dnn_layer->get_K()*dnn_layer->get_G()*STONNE_WORD_SIZE + index_G*dnn_layer->get_K()*STONNE_WORD_SIZE + index_K*STONNE_WORD_SIZE;
            ...
            for(unsigned k=0; k<this->current_tile->get_T_K(); k++) {
                for(unsigned x=0; x<this->current_tile->get_T_X_(); x++) {
                    for(unsigned y=0; y<this->current_tile->get_T_Y_();y++) {
                        ...
                        unsigned int addr_offset = n*this->dnn_layer->get_G()*this->dnn_layer->get_K()*this->dnn_layer->get_X_()*this->dnn_layer->get_Y_()*STONNE_WORD_SIZE + x*this->dnn_layer->get_Y_()*this->dnn_layer->get_G()*this->dnn_layer->get_K()*STONNE_WORD_SIZE + y*this->dnn_layer->get_G()*this->dnn_layer->get_K()*STONNE_WORD_SIZE + g*this->dnn_layer->get_K() + k*STONNE_WORD_SIZE;
...
void SDMemory::setLayer(DNNLayer* dnn_layer, address_t input_address, address_t filter_address, address_t output_address, Dataflow dataflow) {
   ...
   /*
                    ...
                    address_t current_filter_address = filter_address +  k*filter_size*STONNE_WORD_SIZE;
                    address_t current_input_address = input_address + n*input_size*STONNE_WORD_SIZE + x_*row_input_size*STONNE_WORD_SIZE*stride + 
                                                                     y_*dnn_layer->get_C()*STONNE_WORD_SIZE*stride;
                    address_t current_output_address = output_address + n*output_size*STONNE_WORD_SIZE + k*channel_output_size*STONNE_WORD_SIZE +
                                                                     x_*row_output_size*STONNE_WORD_SIZE +  y_*dnn_layer->get_K();
        ...
        if(!this->weights_distributed) {
            ...
            if(current_tile->get_T_N()*current_tile->get_T_X_()*current_tile->get_T_Y_() > 1) { //If N*X_*Y_ is greater than 1 then MULTICAST MESSAGE as the same weight is sent multiple times.
                for(unsigned g=0; g < current_tile->get_T_G(); g++) {
				    ...
				    this->sdmemoryStats.n_SRAM_weight_reads++;
                                    data = filter_address[(index_G+g)*this->group_size*STONNE_WORD_SIZE + (index_K+k)*this->filter_size*STONNE_WORD_SIZE + (index_R+r)*this->row_filter_size*STONNE_WORD_SIZE + (index_S+s)*dnn_layer->get_C()*STONNE_WORD_SIZE + (index_C+c)];  //Fetching. Note the distribution in memory is interleaving the channels
				    }
                ...
                for(unsigned g=0; g < current_tile->get_T_G(); g++) {
				    ...
				    this->sdmemoryStats.n_SRAM_weight_reads++;
                                    ...
                                    data = filter_address[(index_G+g)*this->group_size*STONNE_WORD_SIZE + (index_K+k)*this->filter_size*STONNE_WORD_SIZE + 
				        + (index_R+r)*this->row_filter_size*STONNE_WORD_SIZE + (index_S+s)*dnn_layer->get_C()*STONNE_WORD_SIZE + (index_C+c)];
				    }
            ...
            for(unsigned x=0; x<x_inputs; x++) {
				...
				if(((index_R+x) < output_x_inputs) && ((index_S+y) < (output_y_inputs)) && ((index_C+c) < this->dnn_layer->get_C())) { //Zero-remainder constaint removed
                                this->sdmemoryStats.n_SRAM_input_reads++;
                                data = input_address[(index_N+i)*this->input_size+((index_X*this->dnn_layer->get_strides()+ x) + index_R)*this->dnn_layer->get_Y()*this->dnn_layer->get_C()*this->dnn_layer->get_G()*STONNE_WORD_SIZE+((index_Y*this->dnn_layer->get_strides() + y) + index_S)*this->dnn_layer->get_C()*this->dnn_layer->get_G()*STONNE_WORD_SIZE + (index_G+g)*dnn_layer->get_C()*STONNE_WORD_SIZE + (index_C+c)*STONNE_WORD_SIZE]; //Read value input(x,y)
